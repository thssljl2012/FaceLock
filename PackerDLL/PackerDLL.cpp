...
void deleteFaces(int counter)
{
	for (int i = 1; i <= counter; i++) {
		remove(format("C:\\Face2Lock\\Faces\\s21\\collected_%d.pgm", i).c_str());
	}
}

extern "C" __declspec(dllexport) int FuncInDll (char*);

extern "C" __declspec(dllexport) int FuncInDll (char *bufferLBPH)
{
	...
	for (int i = 1; i <= 20; i++) {
		for (int j = 1; j <= 10; j++) {
			Mat ima = imread(format("C:\\Face2Lock\\Faces\\s%d\\%d.pgm", i, j), 0);
			if (!ima.data) {
	...
	while (true) {
		...
		for (int i = 0; i < faces.size(); i++) {
			...
			labels.push_back(21);
			counter++;
			imwrite(format("C:\\Face2Lock\\Faces\\s21\\collected_%d.pgm", counter), face_resized);
		}
		...
		if (flag == true) {
			...
			if (counter < 20) {
				for (int k = 0; k < variants.size(); k++) {
					...
					labels.push_back(21);
					counter++;
					imwrite(format("C:\\Face2Lock\\Faces\\s21\\collected_%d.pgm", counter), variants[k]);		
				}
			} else {
				for (int k = 0; k < variants.size(); k++) {
	...
	ifstream fin("LBPHXml.xml", ios :: binary);
	fin.read(bufferLBPH + 12, MAX_FILE_SIZE - 1);
	*(int*)(bufferLBPH) = fin.gcount();
	*(double*)(bufferLBPH + 4) = aveConfi / sampleCounter;
	fin.close();
	if (remove("LBPHXml.xml")<0)
		cout << "Error" << endl;

	cout << "Face password is set successfully!" << endl;
